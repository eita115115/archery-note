...
| Pixabay-derived form metric fixtures   | Public Pixabay stock videos documented in `tools/golden-replay/sources.md`                                                                                            | Pixabay Content License                   | Deterministic form-detector regression  | Tracked fields are exactly `tMs`, derived `anchorNorm`/`drawArm`/`bodyScale`/`conf`, and `dWx`/`dWy`/`dWVisibility` (one draw-wrist normalized x/y/visibility time series). No video, pixels, full 33-point landmark set, path, or source URL is included. Updated at 2026-07-26 |
