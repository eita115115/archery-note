...
assert(surface.includes("練習を始める") && surface.includes("missionPanel") && surface.includes("convergeMission") && surface.includes("simplePromise") && surface.includes("quickSelects") && surface.includes("recordSetupSnapshot") && surface.includes("gearWorkbenchHtml"), "v36 lightweight record launch UI missing");
...
new Function(sw);
assert(sw.includes('e.request.mode === "navigate"') && sw.includes('caches.match("./index.html")') && sw.includes("./style.css"), "Service worker navigation fallback missing");
...
assert(html.includes("nativeReadinessHtml") && html.includes("アプリ情報・保存状態") && html.includes("RK4-3D JS core"), "Native readiness UI missing");
assert(html.includes("pageHeroHtml") && html.includes("liveSessionHeroHtml") && html.includes("Growth map") && html.includes("Sight tuning") && html.includes("Equipment lab"), "Reborn workspace UI missing");
