...
**射形トラッキングで射った矢がカウントされない問題を根本修正しました**。実射の診断データで「検出は発火していたのに、その後の処理が射を無言で破棄・誤取消していた」という因果連鎖を確定し、破棄の仕組み自体を運動学的な確認に置き換えました。射数が実際の射数と一致することが、実射プロファイル忠実の合成テスト45本・実映像5本・全既存テストで検証済みです。

### Fixed

- **射形トラッキング — 実射で射がカウントされない問題の根本修正**: 実射診断データ（2026-07-13〜15、4セッション）で真の因果連鎖を確定 — 発火は16回起きていたが、13回が発火後のショット要約 null で無言破棄され、生き残りの一部も誤取消で消失していた。以下の4修正で置換する
  - **ショット要約の段階フォールバック**: `summarizeFormShot` はアンカー登録が遅い射（実測: ANCHORING 出現が発火の 50-70ms 前）で要約窓が空になり null を返してショットを破棄していた。緩ゾーン→全有効フレームの段階フォールバックに変更し、**要約品質とショットのカウントを分離**（角度が出せない射は「—」表示で数える）
  - **出発確認（全発火に普遍適用）**: 従来 null 破棄が偶然担っていたスプリアス発火の抑制を運動学的原理で置換 — 発火後 400ms 以内に手が本当に離れた（anchorNorm≥0.65 が3連続）ことを要求し、ドロー/保持中の誤発火（手が留まる）は自己取消。姿勢ロスで観測できない場合は無罪推定
  - **アンカー復帰取消の時間ベース化**: 実フィールド（conf 0.5-0.7）のランドマーク幻出は2フレーム（33ms）を超えるラン長を持ち実射を誤取消していた（実測 3-4件）。連続ディップ 100ms 以上を要求（真のレットダウン復帰はアンカーに駐留するため影響なし）
  - **NB2（tier-2 ギャップ橋渡し）**: リリース瞬間の姿勢ロス 150〜350ms でアンカー証拠が時効し検出不能だった問題を、sticky アンカーの生存（レットダウンは方向チェックが遮蔽前に必ずクリアする非対称性）＋着地位置・落下・クロスギャップ速度ゲート＋着地後静止確認で橋渡し
  - **loose（誤配置アンカー + 強スパイク）**: 手首ランドマーク誤配置で anchorNorm が 0.35 以上に浮くと速度スパイク（実測 19〜64 tls/s）が見えていても発火不能だった問題を、緩ゾーン（0.6）の静穏保持（vel≤8 が 130ms 以上）と強スパイク（13 tls/s、レットダウン実測上限の4倍超）を条件に発火
  - 検証: 実射プロファイル忠実の合成マトリクス 45 シナリオ（回復系 12 / レットダウン安全系 29 / 既知限界 3 文書化 / スプリアス抑制 1）全緑、実映像ゴールデン再生 5 本回帰、既存境界テスト全緑、E2E 39 本
  - 既知の限界: 350ms を超えるトラッキング欠落は端点情報だけではレットダウンと物理的に判別不能なため対象外（診断データで頻度を監視）

### Changed

- 発火経路の計装 `debug.fireEvidence`（close/loose/nb2）と `debug.fireVel`（vel/nb/nb2）、取消理由 `debug.cancelReason`（anchor-return/nb2-drift/no-depart）を追加。フィールドでどの経路が発火・取消されたかを監査可能に
- 診断データ保存 ON 時の `rejectedFramesNear` 捕捉をフェーズ不問に拡張（スパイク maxV>4 と緩アンカー保持 anchorNorm<0.6。誤配置アンカーは DRAWING/SETUP とラベルされるため従来のフェーズ限定捕捉では診断に写らなかった）

## v1.9.1 - 2026-07-13

### Summary

**開発者向けの調査計装のみのマイナーリリース**。既存ユーザーの検出・判定挙動は完全に不変です。実射で6射中4射が消失した事例を受け、原因を切り分けるための計装データを大幅に拡張しました（設定 > 射形トラッキング > 検証用の診断データ保存 が ON のときのみ蓄積・保存）。
