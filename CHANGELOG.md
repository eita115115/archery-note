...
- ゲーミフィケーションの既定 OFF 補完を、破損したインポート/バックアップ（`settings.gamification` が `null` や非オブジェクトのケース）でも徹底。v1.7.1 の修正が通常起動経路（浅いマージで `blankDb` の既定 false が入る）に依存していたため、この特殊経路だけ古い既定 true が残っていた問題への追い打ち修正。通常起動での挙動変更はありません
