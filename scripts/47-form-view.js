...
function formTrackingEnabled() {
  return !!(db.settings && db.settings.formTrackingEnabled);
}
...
function loadFormPose() {
  if (!formPosePromise) {
    ...
    formPosePromise = import(new URL("vision_bundle.mjs", base).href)
      .then(async (mod) => {
  ...
  const f = {
    ...
    arrowCheck: shot.arrowCheck
      ? {
          ...
          confirmScore:
            shot.arrowCheck.confirmScore == null ? null : +shot.arrowCheck.confirmScore.toFixed(2),
        }
      : null,
  };
...
function formSelfBaselineLabel(value, key, priorRecords) {
  ...
  const priorVals = (priorRecords || [])
    .map((r) => formRecordStats(r))
    .filter(Boolean)
    .map((st) => st[key])
...
function formTrendMiniHtml() {
  ...
  const path = series
    .map((p, i) => `${i ? "L" : "M"}${px(i).toFixed(1)},${py(p.bowArm).toFixed(1)}`)
    .join("");
...
function formScoreLinkHtml() {
  ...
  return `<div class="advice" style="background:var(--card);border-color:var(--line)">
    ...
    ${Math.abs(diff) >= 0.1 ? `<div class="note">${diff > 0 ? `リリースが安定していた日の方が平均 ${diff.toFixed(2)} 点/本 高い傾向です。` : `この期間はドリフトの有無と点数の差が出ていません（他要因が大きい可能性）。`}</div>` : ""}
  </div>`;
}
...
function bindFormTrackingCard() {
  ...
  document.querySelectorAll("[data-form-id]").forEach(
    (li) =>
      (li.onclick = () => {
...
function drawFormSkeleton(ctx, l, w, h) {
  ...
  [
    ...
    L.LEFT_WRIST,
    L.RIGHT_WRIST,
  ].forEach((i) => dot(l[i], i === L.NOSE ? "#ff6c8c" : "#ffe14d"));
}
...
function openFormCapture() {
  ...
  beginActiveWorkflow();
  const video = ovl.querySelector("#fcVideo"),
    canvas = ovl.querySelector("#fcCanvas");
  const ctx = canvas.getContext("2d");
  const hud = ovl.querySelector("#fcHud"),
    phaseEl = ovl.querySelector("#fcPhase");
  ...
  async function shareRec() {
    ...
    try {
      if (navigator.canShare && navigator.canShare({ files: [file] }))
        ...
        const u = URL.createObjectURL(recBlob);
      ...
      if (e.name !== "AbortError") toast("動画の保存に失敗しました");
    }
    recBlob = null;
  }
  ...
  function sampleArrowPresence(raw) {
    ...
    const minX = Math.min(raw.bW.x, raw.dW.x) - pad,
      maxX = Math.max(raw.bW.x, raw.dW.x) + pad;
    const minY = Math.min(raw.bW.y, raw.dW.y) - pad,
      maxY = Math.max(raw.bW.y, raw.dW.y) + pad;
    const sx = Math.max(0, Math.floor(minX * vw)),
      sy = Math.max(0, Math.floor(minY * vh));
    const ex = Math.min(vw, Math.ceil(maxX * vw)),
      ey = Math.min(vh, Math.ceil(maxY * vh));
    const rw = ex - sx,
      rh = ey - sy;
    ...
    let img;
    try {
      img = roiCtx.getImageData(0, 0, rw, rh);
    } catch (e) {
      return null;
    }
  ...
  function loop() {
    ...
    if (landmarker && !cameraSwapInProgress && video.readyState >= 2) {
      ...
      if (cropActive && video.videoWidth) {
        const cw = Math.round(video.videoWidth * CROP_FRAC),
          cx = Math.round(video.videoWidth * CROP_OFF);
        cropCvs.width = cw;
        cropCvs.height = video.videoHeight;
        cropCx.drawImage(video, cx, 0, cw, video.videoHeight, 0, 0, cw, video.videoHeight);
        res = landmarker.detectForVideo(cropCvs, now);
        if (res.landmarks && res.landmarks[0])
          res.landmarks[0].forEach((l) => {
            l.x = l.x * CROP_FRAC + CROP_OFF;
          });
      } else {
      ...
      if (db.settings.formDebug === true && debug) {
        ...
        const isHold =
          (phase === "ANCHORING" ||
            (debug.anchorNorm != null && debug.anchorNorm < FORM_PH.CLOSE_LOOSE)) &&
          now - (lastAnchoringSampleAt || 0) >= 100;
      ...
      const lowFpsHtml =
        fps > 0 && fps < 15
          ? `<br>${icon("warn")} フレームレート低下中（${fps.toFixed(0)}fps）: この端末では検出精度が落ちます`
          : ""; // 提案G2
      if (raw && disp) {
  ...
  ovl.querySelector("#fcSave").onclick = async () => {
    ...
    const rec = {
      ...
      appVer: APP_VER,
      fps: +fps.toFixed(1),
      features: shots.map(formFeatureFromShot),
      note: "",
    };
    ...
    if (hadRec && recBlob) {
      ...
      if (
        await appConfirm("トラッキング動画をカメラロールに保存しますか？", { okLabel: "保存する" })
      )
        await shareRec();
      else recBlob = null;
    }
  ...
  ovl.querySelector("#fcSwap").onclick = async () => {
    ...
    try {
      ...
      hud.textContent = "カメラを切り替えられませんでした: " + e.message;
    } finally {
      cameraSwapInProgress = false;
    }
  };
  ...
  loadFormPose()
    ...
    .catch((e) => {
      hud.textContent =
        "射形解析を開始できませんでした: " +
        ((e && e.message) || e) +
        "（カメラ許可と、iOS 16.4以降/最新ブラウザをご確認ください）";
    });
}
...
function openFormReplay() {
  ...
  input.onchange = () => {
    ...
    if (f) startFormReplay(URL.createObjectURL(f));
  };
  input.click();
}
function startFormReplay(videoUrl) {
  ...
  beginActiveWorkflow();
  const video = ovl.querySelector("#frVideo"),
    canvas = ovl.querySelector("#frCanvas");
  const ctx = canvas.getContext("2d");
  const hud = ovl.querySelector("#frHud"),
    phaseEl = ovl.querySelector("#frPhase");
  let handedness = db.settings.formHandedness === "left" ? "left" : "right";
  let running = true,
    raf = 0,
    landmarker = null;
  ...
  function loop() {
    ...
    if (landmarker && video.readyState >= 2 && !video.paused && !video.ended) {
      ...
      if (now > lastDetectTs) {
        ...
        if (wallNow - lastFpsAt >= 1000) {
          fps = (frames * 1000) / (wallNow - lastFpsAt);
          frames = 0;
          lastFpsAt = wallNow;
        }
        const lms = res.landmarks && res.landmarks[0];
  ...
  loadFormPose()
    .then(async (lm) => {
      ...
      video.preload = "auto";
      video.onerror = () => {
        hud.textContent = "動画の読み込みに失敗しました。対応していない形式の可能性があります。";
      };
      video.onloadeddata = () => {
        canvas.width = video.videoWidth;
        canvas.height = video.videoHeight;
