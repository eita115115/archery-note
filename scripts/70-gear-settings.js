...
const CATALOG_SHAFTS = [
  {
    id: "x10-protour",
    name: "EASTON X10 ProTour",
    aliases: ["X10 PROTOUR", "X10 PRO TOUR", "PROTOUR"],
    type: "carbon",
    dia: 5.0,
    gpiA: 4.7,
    gpiB: 1600,
    sizes: [770, 720, 670, 620, 570, 520, 470, 420, 380, 340],
    gpi: {
      770: 6.0,
      720: 6.2,
      670: 6.5,
  ...
  },
  {
    ...
    gpiA: 3.8,
    gpiB: 1950,
    sizes: [1000, 900, 830, 750, 700, 650, 600, 550, 500, 450, 410, 380, 350, 325],
    gpi: {
      1000: 5.3,
      900: 5.8,
      830: 6.2,
  ...
  {
    id: "x10-32",
    name: "EASTON X10 3.2mm Parallel Pro",
    aliases: ["X10 3.2", "3.2MM PARALLEL PRO"],
    type: "carbon",
    dia: 3.2,
    gpiA: 3.4,
    gpiB: 2200,
    sizes: [1000, 900, 800, 750, 700, 650, 600, 550, 500, 460, 420, 380, 340],
  },
  {
    id: "ace",
    name: "EASTON A/C/E",
  ...
  {
    id: "x10-4",
    name: "EASTON X10 4mm Parallel Pro",
    aliases: ["X10 4MM", "4MM PARALLEL PRO"],
    type: "carbon",
    dia: 4.0,
    gpiA: 4.4,
    gpiB: 2000,
    sizes: [1150, 1000, 880, 810, 710, 660, 610, 570, 520, 470, 420, 380, 340, 300, 250],
  },
  {
    id: "superdrive23",
    name: "EASTON SuperDrive 23",
    aliases: ["SUPERDRIVE 23", "SD23"],
    type: "carbon",
    dia: 9.3,
    gpiA: 7.0,
    gpiB: 900,
  ...
  {
    id: "superdrive-micro",
    name: "EASTON SuperDrive Micro",
    aliases: ["SUPERDRIVE MICRO"],
    type: "carbon",
    dia: 4.0,
    gpiA: 4.9,
    gpiB: 1800,
    sizes: [950, 850, 750, 675, 625, 575, 525, 475, 425, 375, 325],
  },
  {
    id: "avance",
    name: "EASTON Avance",
    ...
    gpiA: 4.1,
    gpiB: 1700,
    sizes: [2000, 1800, 1600, 1400, 1150, 1000, 900, 810, 730, 660, 600, 550, 500, 450, 400, 340],
  },
  {
    id: "avance-sport",
    name: "EASTON Avance Sport",
    aliases: ["AVANCE SPORT"],
    type: "carbon",
    dia: 4.2,
    gpiA: 4.4,
    gpiB: 1600,
  ...
  },
  {
    ...
    gpiA: 3.2,
    gpiB: 1300,
    sizes: [2000, 1800, 1600, 1400, 1200, 1000, 800, 600],
  },
  {
    id: "revelation",
    name: "Black Eagle Revelation",
    aliases: ["REVELATION", "BLACK EAGLE"],
    type: "carbon",
    dia: 5.0,
    gpiA: 5.0,
    gpiB: 1800,
    sizes: [800, 750, 700, 650, 600, 550, 500, 450, 400, 350, 300],
  },
  {
    id: "paragon",
    name: "SKYLON Carbon Paragon",
    aliases: ["PARAGON", "SKYLON"],
    type: "carbon",
    dia: 4.2,
    gpiA: 4.6,
    gpiB: 1800,
    sizes: [1000, 900, 850, 800, 750, 700, 650, 600, 550, 500, 450, 400, 350],
  },
  {
    id: "x23",
    name: "EASTON X23",
    aliases: ["X23"],
    type: "aluminum",
    sizes: [2312, 2314, 2315, 2318],
  },
  {
    id: "x7",
    name: "EASTON X7 Eclipse",
    aliases: ["X7", "ECLIPSE"],
    type: "aluminum",
    sizes: [1514, 1614, 1714, 1814, 1914, 2014, 2114, 2212, 2312],
  },
  {
    id: "xx75pp",
    name: "EASTON XX75 Platinum Plus",
    aliases: ["PLATINUM PLUS", "XX75"],
    type: "aluminum",
    sizes: [1416, 1516, 1616, 1716, 1816, 1916, 2016],
  },
  {
    id: "jazz",
    name: "EASTON XX75 Jazz",
    aliases: ["JAZZ"],
    type: "aluminum",
    sizes: [1214, 1416, 1516, 1616, 1716, 1816, 1916],
  },
];
...
function aluminumDiaMm(code) {
  return (Math.floor(code / 100) / 64) * 25.4;
}
function aluminumGpi(code) {
  const od = Math.floor(code / 100) / 64,
    wall = (code % 100) / 1000,
    id = Math.max(0, od - 2 * wall);
  return (Math.PI / 4) * (od * od - id * id) * 0.0975 * 7000;
}
function carbonGpi(fam, spine) {
  return fam.gpi && fam.gpi[spine] ? fam.gpi[spine] : clamp(fam.gpiA + fam.gpiB / spine, 3, 18);
}
function carbonDiaMm(fam, spine) {
  return fam.odIn && fam.odIn[spine] ? fam.odIn[spine] * 25.4 : fam.dia;
}
function inferCatalogGear(vals) {
  ...
  const fam = [...CATALOG_SHAFTS]
    ...
    .find((f) =>
      f.aliases.some((a) =>
        new RegExp(`(^|\\s)${normGearText(a).replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}(\\s|$)`).test(
          txt,
        ),
      ),
    );
  ...
  const gpiSource =
    fam.type === "aluminum"
      ? "（番手から計算）"
      : fam.gpi && fam.gpi[spine]
        ? "（メーカー掲載値）"
        : "（モデル別近似）";
  const diaSource =
    fam.type === "aluminum"
      ? "（番手から計算）"
      : fam.odIn && fam.odIn[spine]
        ? "（メーカー掲載値）"
        : "（代表値/推定）";
  const notes = [
    `${fam.name} / 番手 ${spine} をカタログ掲載モデルとして認識`,
    `シャフト重量 ${gpi.toFixed(2)}gr/in${gpiSource}`,
    `外径 ${dia.toFixed(2)}mm${diaSource}`,
  ];
  if (!listed)
    notes.push(
      `注意: 番手 ${spine} はこのモデルの掲載サイズ一覧では未確認のため、重量・外径は近似として扱います`,
    );
  if (total)
    notes.push(
      `総矢重量 ${total.toFixed(0)}gr（矢尺${len}in、ポイント${point || 100}gr、ノック/ベイン等27grとして推定）`,
    );
...
const COMMON_SPINES = [
  2000, 1800, 1600, 1400, 1250, 1200, 1150, 1100, 1000, 950, 920, 900, 880, 850, 830, 810, 800, 780,
  750, 730, 720, 710, 700, 675, 670, 660, 650, 625, 620, 610, 600, 575, 570, 550, 525, 520, 500,
  475, 470, 460, 450, 430, 425, 420, 410, 400, 380, 375, 370, 350, 340, 325, 300, 250, 2318, 2315,
  2314, 2312, 2212, 2114, 2016, 2014, 1916, 1914, 1816, 1814, 1716, 1714, 1616, 1614, 1516, 1514,
  1416, 1414, 1214,
];
const GEAR_SUGGESTIONS = {
  bow: [
    "HOYT Grand Prix GMX3 2026 H25",
    "HOYT Grand Prix GMX3 2024 H25",
    "HOYT Grand Prix XCEED 2 H25",
    "HOYT Grand Prix XCEED 2 H27",
    "HOYT Grand Prix XCEED",
    "HOYT Formula SR H25",
    "HOYT Formula XD",
    "HOYT RCRV Podium H25",
    "HOYT RCRV Comp H25",
    "HOYT GMX3",
    "HOYT XCEED 2",
    "WIAWIS ATF-EX H25",
    ...
    "WIAWIS META DX H25",
    "WIAWIS ATF-X H25",
    "WIAWIS RADICAL PRO H25",
    "WIAWIS CX7 H25",
    "WIAWIS INNO CXT H25",
    "WIAWIS INNO CXT H23",
    "WIN&WIN NEW WINEX 2021 H25",
    "WIN&WIN WIAWIS TFT-G",
    "MK ARCHERY X-ON",
    "MK ARCHERY XG",
    "MK KOREA MK ZX",
    "MK ZX",
  ...
  limbs: [
    "HOYT Grand Prix METRIX Foam Core",
    "HOYT Grand Prix METRIX Wood Core",
    "HOYT Formula METRIX Foam Core",
    "HOYT Formula METRIX Wood Core",
    "HOYT Formula AXIA Foam Core",
    "HOYT Formula AXIA Wood Core",
    "HOYT Grand Prix AXIA Wood Core",
    "HOYT Formula RCRV PODIUM Limbs",
    "HOYT Grand Prix RCRV PODIUM Limbs",
    "HOYT Grand Prix COMP Limbs",
    "HOYT Formula Carbon Integra",
    "WIAWIS NS-G2 Foam Core",
    "WIAWIS NS-G2 Wood Core",
    "WIAWIS NS-XP Wood",
    "WIAWIS NS-XP Foam",
    "WIAWIS NS-XP Wood Core ILF",
    "WIAWIS MXT-XP Wood",
    "WIAWIS MXT-XP Foam",
    "WIAWIS MXT-XT Wood",
    "WIAWIS MXT-XT Foam",
    "WIAWIS CX7 Wood Core",
    "WIAWIS CX7 Foam Core",
    "WIAWIS NS-G Wood Core",
    "WIAWIS NS-G Graphene Foam Core",
    "WIN&WIN NEW WINEX Foam Core",
    "MK XD Limbs",
    "MK KOREA ZEST Limbs",
    "MK KOREA N3 Limbs",
    "MK KOREA X-CORE Limbs",
    "WNS Vantage G7 Limbs",
    "WNS Delta C2 Limbs",
    "WNS Explorer W1 Limbs",
    "KINETIC Vaultage Carbon/Foam Limbs",
    "SAMICK Discovery R1 Limbs",
    "SAMICK Polaris Limbs",
    "FIVICS SKADI Limbs Wood Core",
    "FIVICS ARGON-X Limbs Wood Core",
  ],
  sight: [
    "SHIBUYA ULTIMA RC PRO G2 520 Carbon",
    "SHIBUYA ULTIMA RC IV 520 Carbon",
    "SHIBUYA ULTIMA RC PRO G2 Double Mount",
    "SHIBUYA ULTIMA RC PRO 520 Carbon",
    "SHIBUYA Dual Click",
    "SHIBUYA ULTIMA CP PRO G2",
    "SHIBUYA OKULUS Scope",
    "AXCEL Achieve XP Recurve",
    "AXCEL Curve RX Pro",
    "AXCEL Achieve XP Pro Compound",
    "AXCEL AVX",
    "WIAWIS WS700",
    "FIVICS FV-300",
    "FIVICS FV-150",
  ...
  plunger: [
    "SHIBUYA DX Plunger",
    "SHIBUYA Ultima Plunger",
    "Beiter Plunger",
    "WIAWIS ACS Plunger",
    "FIVICS Cushion Plunger",
    "AAE Gold Plunger",
  ],
  nock: [
    "EASTON G Nock",
  ...
  string: [
    ...
    "Angel Majesty 777",
    "Fast Flight Plus",
    "GAS Bowstrings Recurve",
    "GAS Bowstrings High Octane",
    "GAS Bowstrings Ghost XV",
    "FirstString X-IT Wire",
    "FirstString Premium Custom String",
    "ARICO STRING BCY Spectra 652",
    "ARICO STRING Angel Majesty 777",
  ],
  stabilizer: [
    "SHIBUYA VERSA",
    "SHIBUYA PRIMUS",
    "SHIBUYA Vanquish",
    "SHIBUYA Vanquish PRO",
    "EASTON Contour CS",
    "EASTON Halcyon",
    "EASTON Z-Comp",
    "WIAWIS ACS EL",
    ...
    "RAMRODS Ultra V4",
    "RAMRODS BEAST",
    "CONQUEST SmacDown 625",
    "CONQUEST SmacDown 500 Pro",
    "WIN&WIN HMC Plus",
    "FIVICS SKADI-CX",
    "FIVICS V Upper Rod",
    "FIVICS CORE-A Damper",
  ],
  tab: [
    "SHIBUYA APEX",
    "SHIBUYA Tab",
    ...
    "ANGEL Tab 1 Plus Cordovan",
    "ANGEL Tab 2 Plus Cordovan",
    "ANGEL Tab 2 Super Fine Leather",
    "Sherwood Tab Type-1",
    "CARTEL Tab",
    "NEET Glove",
    "Naigai GT-301 Tab",
  ...
  vane: [
    "KOREA ARCHERY Jet6",
    "KOREA ARCHERY Jet6 S",
    "GAS PRO Spin Vanes",
    "XS Wings",
    "Spin Wing",
    "Range-O-Matic Spin Vanes",
    "EliVanes P3",
    "Bohning X Vane",
    "AAE WAV",
    "Kurly Vanes",
...
const GEAR_FIELDS = [
  ...
  ["rest", "レスト"],
  ["string", "弦"],
  ["stabilizer", "スタビライザー/ウエイト"],
  ["tab", "タブ/リリーサー"],
  ["shaftSpine", "シャフト番手/スパイン"],
  ["arrowLength", "矢尺/シャフト長 (inch)"],
  ["pointWeight", "ポイント重量 (gr)"],
  ["arrowDia", "矢の直径 (mm)"],
  ["shaftGpi", "シャフト重量 (gr/in)"],
  ["arrowWeight", "矢重量 合計 (gr)"],
  ["shaftSetWeightSpread", "矢セット重量差 (gr)"],
  ["shaftStraightness", "シャフト真直度 (inch)"],
  ["foc", "FOC (%)"],
  ["vane", "ベイン/羽根"],
  ["vaneHeight", "ベイン高さ (mm)"],
  ["nock", "ノック/ノッキングポイント"],
  ["nockFit", "ノックフィット"],
  ["arrowSpeed", "実測初速 (fps / m/s・任意)"],
  ["arrowCd", "抗力係数 Cd (任意)"],
  ["bowEfficiency", "弓効率 (%)"],
  ["temperature", "気温 (℃)"],
  ["altitude", "標高 (m)"],
  ...
  ["centerShot", "センターショット"],
  ["plunger", "プランジャー"],
  ["plungerTension", "プランジャー硬さ/位置"],
  ["tuningMethod", "確認したチューニング"],
  ["tuningResult", "チューニング結果"],
  ["notes", "その他メモ"],
];
const GEAR_SECTIONS = [
  ...
  {
    title: "物理モデル補正",
    keys: ["arrowSpeed", "arrowCd", "bowEfficiency", "temperature", "altitude", "humidity"],
  },
  { title: "メモ", keys: ["notes"] },
];
const GEAR_NUMERIC_FIELDS = new Set([
  "poundage",
  "drawLength",
  "shaftSpine",
  "arrowLength",
...
function setChoiceValue(root, id, value) {
  ...
  const match = [...pick.options].find(
    (o) => o.value && o.value !== MANUAL_CHOICE && normGearText(o.value) === normGearText(val),
  );
  if (match) {
...
const GEAR_UNIT_LABELS = {
  ...
  shaftGpi: "gr/in",
  arrowWeight: "gr",
  shaftSetWeightSpread: "gr",
  shaftStraightness: "inch",
  foc: "%",
  vaneHeight: "mm",
  bowEfficiency: "%",
  temperature: "℃",
...
function gearFieldHtml(k, lb, s) {
  ...
  if (k === "notes")
    return `<label class="f" for="gf_${k}">${lb}</label><textarea class="inp" id="gf_${k}"${ph}>${esc(s ? s[k] : "")}</textarea>`;
  if (opts.length && !GEAR_NUMERIC_FIELDS.has(k))
    return choiceFieldHtml(`gf_${k}`, lb, opts, s ? s[k] : "", GEAR_PLACEHOLDERS[k] || "", mode);
  const { label, unit } = gearLabelUnit(k, lb);
  const input = `<input class="inp" id="gf_${k}" value="${esc(s ? s[k] : "")}"${list}${mode}${ph}>`;
  const body = unit
    ? `<div class="fieldUnit">${input}<span class="fieldUnitLabel">${esc(unit)}</span></div>`
    : input;
...
function gearPrecisionProfile(s) {
  const checks = [
    ...
    ["drawLength", "引き尺"],
    ["arrowSpeed", "実測初速"],
    ["shaftSetWeightSpread", "矢セット重量差"],
    ["foc", "FOC"],
    ["vane", "ベイン"],
    ["tuningMethod", "チューニング確認"],
    ["temperature", "気温/環境"],
  ];
  ...
  const level = score >= 0.78 ? "高" : score >= 0.5 ? "中" : "低";
...
function spineGuidance(s) {
  ...
  if (p == null || len == null)
    return {
      ready: false,
      missing: [p == null ? "実測ポンド" : null, len == null ? "矢尺" : null].filter(Boolean),
    };
  ...
  let state = "候補を表示",
    tone = "mid",
    text = "カタログ表で最終確認してください。";
  if (actual != null) {
    const aluminum = actual > 1200 && actual % 100 >= 10 && actual % 100 <= 20;
    if (aluminum) {
      ((state = "アルミ番手"),
        (text = "アルミ番手はカーボンスパインと体系が違うため、番手表と実射で確認します。"));
    } else {
      const diff = actual - ideal;
      if (Math.abs(diff) <= 90) {
        state = "概ね候補域";
        tone = "ok";
        text = "現在の番手は初期候補レンジに入っています。";
      } else if (diff > 90) {
        state = "柔らかめ寄り";
        tone = "warn";
        text =
          "現在の番手は初期候補より柔らかめの可能性があります。矢飛びとチューニングを確認してください。";
      } else {
        state = "硬め寄り";
        tone = "warn";
        text =
          "現在の番手は初期候補より硬めの可能性があります。矢飛びとチューニングを確認してください。";
      }
    }
  }
...
function spineGuidanceHtml(s) {
  ...
  if (!g.ready)
    return `<div class="note">スパイン初期候補: ${esc(g.missing.join("・"))}未入力</div>`;
  const color =
    g.tone === "ok"
      ? "var(--status-ok)"
      : g.tone === "warn"
...
function setupComparisonHtml(setupId) {
  const ss = db.sessions
    .filter((s) => s.setupId === setupId)
    .sort((a, b) => (a.date || "").localeCompare(b.date || "") || (a.id > b.id ? 1 : -1));
  if (ss.length < 2) return "";
  const last = ss[ss.length - 1];
  const prev =
    [...ss.slice(0, -1)].reverse().find((s) => s.dist === last.dist) || ss[ss.length - 2];
  const metric = (s) => {
  ...
  const delta = (x, unit = "", goodLow = false) => {
    ...
    const color =
      Math.abs(x) < 0.01 ? "var(--sub)" : good ? "var(--status-ok)" : "var(--status-warn)";
  ...
  return `<div class="advice gearAdviceCard">
    ...
    ${
      a.st && b.st
        ? `<div class="kv"><span>グルーピング半径</span><span>${a.st.rr.toFixed(1)}cm → ${b.st.rr.toFixed(1)}cm（${delta(b.st.rr - a.st.rr, "cm", true)}）</span></div>
...
function gearHeroHtml() {
  ...
  const lastSess = [...db.sessions].sort(
...
function renderGear(m) {
  ...
  document
    .querySelectorAll("#gearList .listItem")
    .forEach((li) => (li.onclick = () => openGearDetail(li.dataset.id)));
}

/* ---------- カスタムラウンド（IMP-09 多距離ラウンド定義） ---------- */
/* 的の選択肢は parseFaceChoice の語彙（122/80/60/40 単一・T40 三つ目・F〜 フィールド）に合わせる */
const CUSTOM_ROUND_FACES = [
  ["122", "122cm"],
  ["80", "80cm"],
  ["60", "60cm"],
  ["40", "40cm"],
  ...
  return `<details class="adv" data-testid="settings-custom-rounds"><summary>多距離ラウンドの定義を管理</summary>
    ...
    ${
      list.length
        ? list
            .map(
              (r) => `<button type="button" class="listItem" data-cr="${esc(r.id)}"><div>
      <div class="t">${esc(r.label)}</div>
      <div class="d">${esc(customRoundStagesText(r))}</div>
    </div><div class="gearChevron">${icon("chevron")}</div></button>`,
            )
            .join("")
        : `<div class="empty">カスタムラウンドはまだありません。</div>`
    }
...
function openCustomRoundForm(id) {
  ...
  const stages = (
    src ? src.stages : [{ dist: 70, faceD: 122, faceType: "single", arrows: 36, perEnd: 6 }]
  ).map((st) => ({
    dist: st.dist,
    face: faceChoiceValue(st),
    arrows: st.arrows,
    perEnd: st.perEnd || 6,
  }));
  ...
  function renderStages() {
    ...
    ovl.querySelectorAll("[data-st-dist]").forEach(
      (inp) =>
        (inp.onchange = (e) => {
          stages[+inp.dataset.stDist].dist = e.target.value.trim();
        }),
    );
    ovl.querySelectorAll("[data-st-face]").forEach(
      (sel) =>
        (sel.onchange = (e) => {
          stages[+sel.dataset.stFace].face = e.target.value;
        }),
    );
    ovl.querySelectorAll("[data-st-arrows]").forEach(
      (inp) =>
        (inp.onchange = (e) => {
          stages[+inp.dataset.stArrows].arrows = e.target.value.trim();
        }),
    );
    ovl.querySelectorAll("[data-st-perend]").forEach(
      (sel) =>
        (sel.onchange = (e) => {
          stages[+sel.dataset.stPerend].perEnd = +e.target.value || 6;
        }),
    );
    ovl.querySelectorAll("[data-del-stage]").forEach(
      (b) =>
        (b.onclick = () => {
          stages.splice(+b.dataset.delStage, 1);
          renderStages();
        }),
    );
  }
  ...
  ovl.querySelector("#crAddStage").onclick = () => {
    ...
    stages.push({
      dist: "",
      face: prev ? prev.face : "122",
      arrows: prev ? prev.arrows : 36,
      perEnd: prev ? prev.perEnd : 6,
    });
    renderStages();
  };
  ovl.querySelector("#crCancel").onclick = () => {
    closeModal(ovl);
    openSettings();
  };
  ovl.querySelector("#crSave").onclick = async () => {
    ...
    for (let i = 0; i < stages.length; i++) {
      const dist = +stages[i].dist,
        arrows = +stages[i].arrows,
        perEnd = +stages[i].perEnd || 6;
      if (!(dist > 0 && arrows > 0)) {
        toast(`ステージ${i + 1}の距離と射数を入力してください`);
        return;
      }
      if (!Number.isInteger(arrows)) {
        toast(`ステージ${i + 1}の射数は正の整数で入力してください`);
        return;
      }
      if (perEnd > arrows) {
        toast(`ステージ${i + 1}の1エンドの本数（${perEnd}本）が射数（${arrows}射）を超えています`);
        return;
      }
    ...
    if (
      src &&
      db.active &&
      db.active.roundGroup &&
      db.active.roundGroup.roundId === id &&
      !(await appConfirm(
        "記録中のラウンドがあり、変更するとステージ進行が完了できなくなる可能性があります。保存しますか？",
        { okLabel: "保存する" },
      ))
    )
      return;
    ...
    if (src) {
      ...
      if (i >= 0) db.customRounds[i] = { id, label, stages: clean };
      else db.customRounds.push({ id, label, stages: clean });
    } else {
    ...
    closeModal(ovl);
    openSettings();
    toast("カスタムラウンドを保存しました");
  };
  const del = ovl.querySelector("#crDel");
  if (del)
    del.onclick = async () => {
      const activeWarn =
        db.active && db.active.roundGroup && db.active.roundGroup.roundId === id
          ? "\n記録中のラウンドがあり、削除するとステージ進行が完了できなくなる可能性があります。"
          : "";
      if (
        await appConfirm(
          `「${src.label}」を削除しますか？${activeWarn}\n（過去の練習記録は残ります。記録タブの選択肢から消えます）`,
...
function openSettings() {
  ...
  ovl.innerHTML = `<div class="sheet"><h3>${icon("gear")} 設定</h3>
    ...
    <div class="settingsGroup" data-testid="settings-group-display">
      ...
      <div class="chips" id="thChips" data-testid="settings-theme-chips">
        ${[
          ...
          .map(
            ([v, lb]) =>
              `<button type="button" class="chip ${th === v ? "on" : ""}" aria-pressed="${th === v}" data-th="${v}">${lb}</button>`,
          )
          .join("")}
      </div>
    ...
    <div class="settingsGroup" data-testid="settings-group-data">
      ...
      ${db.sessions.some((s) => String(s.id || "").startsWith(DEMO_ID_PREFIX)) ? `<div class="settingsActionHint">架空のデモ記録だけを削除します。自分の記録には触れません。</div><div class="btnrow"><button class="btn ghost" id="dDemoRemove" data-testid="settings-demo-remove">デモデータを完全に削除</button></div>` : ""}
  ...
  ovl.querySelectorAll("#thChips .chip").forEach(
    (c) =>
      (c.onclick = () => {
        db.settings.theme = c.dataset.th;
        save();
        applyTheme();
        applyFieldMode();
        ovl.querySelectorAll("#thChips .chip").forEach((x) => {
          const on = x === c;
  ...
  ovl.querySelectorAll("#fmChips .chip").forEach(
    (c) =>
      (c.onclick = () => {
        db.settings.fieldMode = c.dataset.fm === "1";
        save();
        applyFieldMode();
        ovl.querySelectorAll("#fmChips .chip").forEach((x) => {
          const on = x === c;
          x.classList.toggle("on", on);
          x.setAttribute("aria-pressed", String(on));
        });
  ...
  ovl.querySelector("#setEye").onchange = (e) => {
    db.settings.eyeSight = +e.target.value || 850;
    save();
  };
  ovl.querySelectorAll("#ftChips .chip").forEach(
    (c) =>
      (c.onclick = () => {
        db.settings.formTrackingEnabled = c.dataset.ft === "1";
        save();
        ovl.querySelectorAll("#ftChips .chip").forEach((x) => {
          const on = x === c;
          x.classList.toggle("on", on);
          x.setAttribute("aria-pressed", String(on));
        });
  ...
  ovl.querySelectorAll("[data-cr]").forEach(
    (li) =>
      (li.onclick = () => {
        closeModal(ovl);
        openCustomRoundForm(li.dataset.cr);
      }),
  );
  ...
  const demoRemove = ovl.querySelector("#dDemoRemove");
  if (demoRemove)
    demoRemove.onclick = async () => {
      if (
        await appConfirm("架空のデモデータだけを完全に削除しますか？", {
          danger: true,
          okLabel: "削除する",
        })
      ) {
        const count = removeDemoData();
        closeModal(ovl);
        render();
        toast(`デモデータを${count}件削除しました`);
      }
    };
  ovl.querySelector("#dSnapNow").onclick = () => {
    writeSafetySnapshot("manual", true);
    toast("現在のデータをバックアップしました");
    closeModal(ovl);
    openSettings();
  };
  ovl.querySelector("#dSnapRestore").onclick = async () => {
    ...
    if (!snap || !snap.data) {
      toast("復元できるバックアップデータがありません");
      return;
    }
    if (
      await appConfirm(
        `${snapshotLabel(snap)} を復元します。\n現在のデータも先にバックアップしてから置き換えます。よろしいですか？`,
  ...
  ovl.querySelector("#dFile").onchange = (e) => {
    ...
    r.onload = async () => {
      try {
        ...
        if (
          await appConfirm(
            `読み込むと現在のデータは置き換わります。\n（練習${(d.sessions || []).length}回 / セッティング${(d.setups || []).length}件）${warnText}よろしいですか？`,
            { danger: true, okLabel: "読み込む" },
          )
        ) {
          ...
          if (db.settings.gamification && db.settings.gamification.enabled) {
            try {
              ...
              console.warn(
                "[gamification] backfill on import failed, will retry on next launch",
                e,
              );
            }
          }
          save({ reason: "import", forceSnapshot: true });
          applyTheme();
          applyFieldMode();
          closeModal(ovl);
          render();
  ...
  ovl.querySelectorAll("[data-restore-trash]").forEach(
    (b) =>
      (b.onclick = () => {
        beginActiveWorkflow();
        try {
          if (restoreTrash(b.dataset.restoreTrash)) {
            closeModal(ovl);
            render();
            openSettings();
            toast("復元しました");
  ...
  if (tc)
    tc.onclick = async () => {
      if (
        await appConfirm("ゴミ箱の中身を完全に削除しますか？", {
          danger: true,
          okLabel: "空にする",
        })
      ) {
        db.trash = [];
        save({ reason: "clear-trash", forceSnapshot: true });
        closeModal(ovl);
        openSettings();
        toast("ゴミ箱を空にしました");
      }
...
function openSetupWizard() {
  ...
  ovl.querySelector("#wSave").onclick = () => {
    ...
    [70, 50, 30, 18].forEach((d) => {
      const v = ovl.querySelector(`#wV_${d}`).value.trim(),
        h = ovl.querySelector(`#wH_${d}`).value.trim();
      if (v || h)
        db.sightMarks.push({
          id: uid(),
          setupId: n.id,
    ...
    save({ reason: "setup-wizard", forceSnapshot: true });
    closeModal(ovl);
    render();
    toast("初回セットアップを保存しました");
  };
}
function openGearDetail(id) {
  ...
  ovl.innerHTML = `<div class="sheet"><h3>${esc(s.name)}</h3>
    ...
    ${
      s.history && s.history.length
        ? `<h3 class="gearHistoryH3">変更履歴</h3>
      ${[...s.history]
        .reverse()
        .map(
          (h) => `<div class="gearHistoryRow">
        <b>${fmtD(h.date)}</b> — ${h.changes.map((c) => `${esc(c.label)}: ${esc(c.from || "（未設定）")} → <b>${esc(c.to || "（削除）")}</b>`).join(" / ")}</div>`,
        )
        .join("")}`
        : ""
    }
  ...
  ovl.querySelector("#gEdit").onclick = () => {
    closeModal(ovl);
    openGearForm(id);
  };
...
function openGearForm(id) {
  ...
  ovl.querySelector("#gfInfer").onclick = () => {
    ...
    if (!inf) {
      hint.textContent =
        "カタログ推定できるシャフト名が見つかりませんでした。例: EASTON X10 / EASTON A/C/E / EASTON Avance / EASTON XX75 Platinum Plus";
      toast("シャフト名を読み取れません");
      return;
    }
    if (inf.missing) {
      hint.textContent = `${inf.fam.name} は見つかりました。番手/スパイン欄も入力してください。`;
      toast("番手を追加してください");
      return;
    }
  ...
  ovl.querySelector("#gfSave").onclick = () => {
    ...
    if (s) {
      ...
      s.name = name;
      if (changes.length) {
        (s.history = s.history || []).push({ date: today(), changes });
        toast("変更を履歴に記録しました");
      }
    } else {
      const n = { id: uid(), name, history: [], createdAt: today() };
