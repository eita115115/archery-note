...
開けるための設計と手順を定義する。`docs/tracking-analysis-plan.md`（v1.1.0 で完了）の
続編にあたる。
...
- 対象: リポジトリ外の作業フォルダ（例 `~/Projects/pose-prototype/`）。
  リポジトリには成果レポート `docs/form-tracking-feasibility.md` のみ追加
...
- 対象ファイル: `docs/storage-schema4-design.md`（新規）、
...
- 完了条件: `npm run check:storage` pass。設計 doc が
  `docs/storage-migration-safety-checklist.md` の全ゲートに1対1で答えている
...
- 対象ファイル: `assets/pose/`（`pose_landmarker_lite.task` + tasks-vision の
  ...
  `docs/form-tracking-assets.md`（取得手順の再現メモ）
...
- 概要のみ: settings.formTrackingEnabled フラグ、カメラ許可フロー、
  ...
  F1〜F4 の結果を踏まえて `docs/tracking-analysis-plan.md` と同形式の
  タスク分解を別途作成する
